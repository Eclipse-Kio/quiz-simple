const REPLACEMENT = "DEADBEAF"
function parse(src) {
    let result = ""
    let pointerIndex = 0;
    let currentWord = "";

    while (pointerIndex < src.length) {
        const currentChar = src.charAt(pointerIndex);
        if (isLetter(currentChar)) {//是字母,直接加入当前词汇
            currentWord += currentChar;
            if (pointerIndex == src.length - 1) {//注意到末尾一个单词需要再处理一下
                if (needReplace(currentWord)) {
            ...
            currentWord += currentChar;
            if (currentChar == " ") {//碰到空格说明上一个单词处理完毕，加入result，
                result += currentWord;
                currentWord = "";
            }
        }
...
function isLetter(char) {
    const mChar = char.toUpperCase()
    return mChar >= 'A' && mChar <= 'Z';
}

function needReplace(src) {
    return src.endsWith("am") || src.endsWith("em") || src.endsWith("im") || src.endsWith("om") || src.endsWith("um");
}

const raw = `Maxime facere magni esse enim ipsa suscipit optio deserunt nobis a vel voluptate
...
let start = Date.parse(new Date()), end = 0;
const result = parse(raw)
end = Date.parse(new Date());
console.log(result);

document.querySelector('#timeUsed').innerText = end - start + " Mill Seconds";
